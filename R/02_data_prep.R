        ...
        prep_dyad <- function(df) {
                df %>%
                        mutate(
                                ...
                                # Safe legitimation ratio (avoid division by zero, negatives, overflow)
                                legit_total = coalesce(v2exl_legitideol_a, 0) + 
                                        coalesce(v2exl_legitperf_a, 0) + 
                                        coalesce(v2exl_legitlead_a, 0),
                                
                                legit_ratio = case_when(
                                        legit_total <= 0 | is.na(v2exl_legitideol_a) | is.na(v2exl_legitperf_a) | is.na(v2exl_legitlead_a) ~ NA_real_,
                                        TRUE ~ v2exl_legitideol_a / legit_total
                                ),
                                
                                # Clamp to realistic range [0,1] (safety net)
                                legit_ratio = pmin(pmax(legit_ratio, 0), 1),
                        ...
                        mutate(
                                ...
                                t2 = t^2,
                                t3 = t^3,
                                # Scaled temporal terms (Carter & Signorino recommendation)
                                t_scaled = t / 100,
