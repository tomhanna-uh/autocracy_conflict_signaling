...
# ==============================================================================
# Helper: safe VIF for Cox models
# ==============================================================================
safe_vif_cox <- function(model, label = deparse(substitute(model))) {
  tryCatch({
    v <- car::vif(model)
    if (is.matrix(v)) v <- v[, "GVIF"]
    message(sprintf("[06] VIF (%s): max = %.2f", label, max(v, na.rm = TRUE)))
    v
  }, error = function(e) {
    message(sprintf("[06] VIF failed for %s: %s", label, e$message))
    NULL
  })
}
...
# 3. VIF Diagnostics ----
# ==============================================================================
h9_vif <- lapply(setNames(names(h9_survival), names(h9_survival)),
                 function(nm) safe_vif_cox(h9_survival[[nm]], nm))

dir.create("results", showWarnings = FALSE)
saveRDS(h9_survival, "results/h9_survival.rds")
saveRDS(h9_vif,      "results/h9_vif.rds")

rm(h9_data, h9_vars, h9_survival, h9_vif)
gc()
