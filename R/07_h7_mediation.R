...
# Run mediation first (needs unstripped models)
h7_mediation  <- run_h7_mediation(h7_components)
h7_sem        <- estimate_h7_sem(h7_data)

# ==============================================================================
# 4. VIF Diagnostics (on component lm/glm models before stripping) ----
# ==============================================================================
h7_vif <- NULL
if (!is.null(h7_components)) {
  h7_vif <- list(
    a_religious = safe_vif(h7_components$a_religious, "a_religious"),
    b_religious = safe_vif(h7_components$b_religious, "b_religious"),
    a_party     = safe_vif(h7_components$a_party,     "a_party"),
    b_party     = safe_vif(h7_components$b_party,     "b_party"),
    a_military  = safe_vif(h7_components$a_military,  "a_military"),
    b_military  = safe_vif(h7_components$b_military,  "b_military")
  )
}
...
saveRDS(h7_vif,        "results/h7_vif.rds")

rm(h7_data, h7_vars, h7_components, h7_mediation, h7_sem, h7_vif)
gc()
