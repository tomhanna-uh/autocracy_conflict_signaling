# ==============================================================================
# 09_reporting_tables.R -- Consolidated Results Reporting
# Robust version that handles models with different predictors
# (e.g., after removing cold_war or support dummies)
# ==============================================================================

source(here::here("R", "00_packages.R"))

# ==============================================================================
# 1. Utility Functions (ROBUST VERSION) ----
# ==============================================================================

#' Safe vcov that works with stripped brglmFit models
safe_vcov <- function(model) {
        if (is.null(model)) return(NULL)
        if (inherits(model, "brglmFit") && !is.null(model$Fisher.info)) {
                V <- solve(model$Fisher.info)
                nms <- names(coef(model))
                if (nrow(V) == length(nms)) {
                        rownames(V) <- colnames(V) <- nms
                }
                return(V)
        }
        if (inherits(model, "glm") && !is.null(model$qr)) {
                p <- model$rank
                p1 <- seq_len(p)
                R <- qr.R(model$qr)[p1, p1, drop = FALSE]
                Rinv <- backsolve(R, diag(p))
                dispersion <- if (model$family$family %in% c("binomial", "poisson")) 1 else {
                        sum(model$weights * model$residuals^2) / model$df.residual
                }
                V <- dispersion * tcrossprod(Rinv)
                rownames(V) <- colnames(V) <- names(coef(model))[p1]
                return(V)
        }
        stats::vcov(model)
}

#' Safe coefficient extractor (works even if some models are NULL)
extract_coefs <- function(model, model_name = "") {
        if (is.null(model) || length(coef(model)) == 0) return(NULL)
        
        tryCatch({
                beta <- coef(model)
                v <- tryCatch(safe_vcov(model), error = function(e) NULL)
                se <- if (is.null(v)) rep(NA_real_, length(beta)) else sqrt(diag(v))
                z <- beta / se
                pval <- 2 * pnorm(abs(z), lower.tail = FALSE)
                
                data.frame(
                        model = model_name,
                        term = names(beta),
                        estimate = beta,
                        std.error = se,
                        p.value = pval,
                        stringsAsFactors = FALSE
                )
        }, error = function(e) {
                message("Could not extract from ", model_name, ": ", e$message)
                NULL
        })
}

#' Robust wide table builder — handles different predictors across models
build_model_table <- function(model_list, nice_names = NULL) {
        if (length(model_list) == 0) return(NULL)
        
        # Extract coefficients
        coef_dfs <- lapply(names(model_list), function(nm) {
                extract_coefs(model_list[[nm]], nm)
        })
        names(coef_dfs) <- names(model_list)
        
        # Drop NULL models
        coef_dfs <- coef_dfs[!sapply(coef_dfs, is.null)]
        if (length(coef_dfs) == 0) {
                message("No valid model coefficients could be extracted.")
                return(NULL)
        }
        
        # Get all unique terms
        all_terms <- unique(unlist(lapply(coef_dfs, `[[`, "term")))
        
        # Start wide table
        wide <- data.frame(label = all_terms, stringsAsFactors = FALSE)
        
        for (mdl in names(coef_dfs)) {
                df <- coef_dfs[[mdl]]
                wide[[mdl]] <- NA_character_
                
                idx <- match(wide$label, df$term)
                valid <- !is.na(idx)
                
                if (any(valid)) {
                        est   <- df$estimate[idx[valid]]
                        se    <- df$std.error[idx[valid]]
                        p     <- df$p.value[idx[valid]]
                        stars <- ifelse(p < 0.001, "***",
                                        ifelse(p < 0.01, "**",
                                               ifelse(p < 0.05, "*", "")))
                        
                        wide[[mdl]][valid] <- paste0(sprintf("%.3f", est), stars,
                                                     " (", sprintf("%.3f", se), ")")
                }
        }
        
        # Optional nice term names
        if (!is.null(nice_names)) {
                wide$label <- nice_names[match(wide$label, names(nice_names))]
                wide$label[is.na(wide$label)] <- wide$label[is.na(wide$label)]
        }
        
        wide
}

#' Write table to console + HTML
write_table <- function(tbl, title, filepath) {
        if (is.null(tbl) || nrow(tbl) == 0) {
                message(sprintf("[09] Skipping %s (no valid data)", title))
                return(invisible(NULL))
        }
        
        cat(sprintf("\n=== %s ===\n", title))
        print(knitr::kable(tbl, format = "simple", row.names = FALSE))
        
        html <- knitr::kable(tbl, format = "html", row.names = FALSE, caption = title)
        writeLines(as.character(html), filepath)
        message(sprintf("[09] Wrote: %s", filepath))
}

# ==============================================================================
# 2. Load Results ----
# ==============================================================================
load_all_results <- function() {
        safe_load <- function(path) {
                if (file.exists(path)) readRDS(path) else { warning(sprintf("[09] Missing: %s", path)); NULL }
        }
        list(
                h1 = safe_load("results/h1_logit_models.rds"),
                h2 = safe_load("results/h2_logit_models.rds"),
                h3 = safe_load("results/h3_logit_models.rds"),
                h4 = safe_load("results/h4_logit_models.rds"),
                h5 = safe_load("results/h5_logit_models.rds"),
                h6 = safe_load("results/h6_logit_models.rds"),
                h8_init = safe_load("results/h8_init_models.rds"),
                h8_target = safe_load("results/h8_target_models.rds"),
                h9 = safe_load("results/h9_survival.rds")
        )
}

# ==============================================================================
# 3. Reporting Functions (with H3/H4 adapted) ----
# ==============================================================================

report_tier1_logit <- function(res) {
        dir.create("results/tables", recursive = TRUE, showWarnings = FALSE)
        
        # H1 Main
        if (!is.null(res$h1)) {
                tbl <- build_model_table(list(
                        "H1 Baseline" = res$h1$h1_baseline,
                        "H1 Controls" = res$h1$h1_controls,
                        "H1 Full"     = res$h1$h1_full
                ))
                write_table(tbl, "Tier 1: Leader Ideology and Conflict Initiation (H1)",
                            "results/tables/tier1_h1_main.html")
        }
        
        # H1 Sub-types
        if (!is.null(res$h1)) {
                tbl <- build_model_table(list(
                        "Religious (base)"    = res$h1$h1_religious_base,
                        "Religious (+ctrl)"   = res$h1$h1_religious,
                        "Socialist (base)"    = res$h1$h1_socialist_base,
                        "Socialist (+ctrl)"   = res$h1$h1_socialist,
                        "Nationalist (base)"  = res$h1$h1_nationalist_base,
                        "Nationalist (+ctrl)" = res$h1$h1_nationalist
                ))
                write_table(tbl, "H1: Ideology Sub-types and Conflict Initiation",
                            "results/tables/tier1_h1_subtypes.html")
        }
        
        # H2
        if (!is.null(res$h2)) {
                tbl <- build_model_table(list(
                        "H2 Baseline" = res$h2$h2_baseline,
                        "H2 Controls" = res$h2$h2_controls,
                        "H2 Full"     = res$h2$h2_full
                ))
                write_table(tbl, "Tier 1: Leader Ideology and Democracy Targeting (H2)",
                            "results/tables/tier1_h2_main.html")
        }
        
        # === H3 & H4 Support Group Results (adapted version) ===
        if (!is.null(res$h3) && !is.null(res$h4)) {
                message("[09] Building H3 & H4 tables...")
                
                # H3 models
                h3_models <- list(
                        "H3 Baseline" = res$h3$h3_baseline,
                        "H3 Party"    = res$h3$h3_party,
                        "H3 Military" = res$h3$h3_military,
                        "H3 Multi"    = res$h3$h3_multi,
                        "H3 Controls" = res$h3$h3_controls,
                        "H3 Full"     = res$h3$h3_full
                )
                
                # H4 models
                h4_models <- list(
                        "H4 Baseline" = res$h4$h4_baseline,
                        "H4 Party"    = res$h4$h4_party,
                        "H4 Military" = res$h4$h4_military,
                        "H4 Multi"    = res$h4$h4_multi,
                        "H4 Controls" = res$h4$h4_controls,
                        "H4 Full"     = res$h4$h4_full
                )
                
                tbl_h3 <- build_model_table(h3_models)
                tbl_h4 <- build_model_table(h4_models)
                
                if (!is.null(tbl_h3) && !is.null(tbl_h4)) {
                        tbl <- cbind(tbl_h3, tbl_h4[,-1, drop = FALSE])  # combine without duplicate label column
                } else if (!is.null(tbl_h3)) {
                        tbl <- tbl_h3
                } else if (!is.null(tbl_h4)) {
                        tbl <- tbl_h4
                } else {
                        tbl <- NULL
                }
                
                write_table(tbl, "Tier 1: Support Group Ideology (H3 & H4)",
                            "results/tables/tier1_h3_h4_main.html")
                # After write_table() for H3/H4
                cat("\nNote: Full models exclude capabilities and coalition size controls to focus on support-group effects.\n", file = filepath, append = TRUE)
        } else {
                message("[09] Skipping Tier 1: Support Group Ideology (H3 & H4) (no data)")
        }
}

report_tier2_legit <- function(res) {
        if (!is.null(res$h5) && !is.null(res$h6)) {
                tbl <- build_model_table(list(
                        "H5 Full" = res$h5$h5_full,
                        "H6 Full" = res$h6$h6_full
                ))
                write_table(tbl, "Tier 2: Legitimation Mix and Conflict (H5 & H6)",
                            "results/tables/tier2_legit_mix.html")
        }
}

report_tier3_complex <- function(res) {
        if (!is.null(res$h8_init) && !is.null(res$h8_target)) {
                tbl <- build_model_table(list(
                        "Init (Int)"   = res$h8_init$h8_init_int,
                        "Target (Int)" = res$h8_target$h8_target_int
                ))
                write_table(tbl, "Tier 3: Moderation by Dynamic Leadership (H8)",
                            "results/tables/tier3_h8_moderation.html")
        }
        
        if (!is.null(res$h9)) {
                tbl <- build_model_table(list(
                        "Direct"      = res$h9$cox_h9_direct,
                        "+Ratio"      = res$h9$cox_h9_ratio,
                        "Interaction" = res$h9$cox_h9_int_ratio
                ))
                write_table(tbl, "Tier 3: Leader Survival and Conflict (H9)",
                            "results/tables/tier3_h9_survival.html")
        }
}

# ==============================================================================
# 4. Execution ----
# ==============================================================================
try({
        all_res <- list(
                h1 = readRDS("results/h1_logit_models.rds"),
                h2 = readRDS("results/h2_logit_models.rds"),
                h3 = readRDS("results/h3_logit_models.rds"),
                h4 = readRDS("results/h4_logit_models.rds"),
                h5 = readRDS("results/h5_logit_models.rds"),
                h6 = readRDS("results/h6_logit_models.rds"),
                h8_init = readRDS("results/h8_init_models.rds"),
                h8_target = readRDS("results/h8_target_models.rds"),
                h9 = readRDS("results/h9_survival.rds")
        )
        
        report_tier1_logit(all_res)
        report_tier2_legit(all_res)
        report_tier3_complex(all_res)
        
        message("[09_reporting_tables.R] All reporting tables generated in results/tables/")
}, silent = FALSE)
